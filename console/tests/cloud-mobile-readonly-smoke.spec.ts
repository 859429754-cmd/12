...
test.describe("真实云端控制台移动端只读 smoke", () => {
  test.skip(Boolean(missingCloudReadonlyConfig()), missingCloudReadonlyConfig());

  test.beforeEach(async ({ page }) => {
    await page.route("**/api/control/**", async (route) => {
      throw new Error(`cloud_mobile_readonly_smoke_must_not_call_control_api:${route.request().url()}`);
    });
  });

  test("账号1移动端总览、新闻和行情图表可用", async ({ page }) => {
  ...
  test("账号2移动端只读 follower 视图可用", async ({ page }) => {
  ...
  test("管理员移动端交易与 AI 页面可读但不触发控制 API", async ({ page }) => {
