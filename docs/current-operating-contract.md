...
## 2026-07-13 AI 增强 / 纯策略双模式合同

以后以本节和 ADR-0006 为准，忽略之前“管理员关闭 DeepSeek 后 live 新开仓一律 fail-closed”的方案。

```text
ai.enabled=true  -> AI 增强模式
ai.enabled=false -> 纯策略模式
```

纯策略模式不调用 DeepSeek，不使用 AI 五档、新闻/订单流仓位裁剪、AI 漂移阻断和持仓复评加仓；入场数量使用本地 KC+VOL+KDJ 策略建议量，再经过开仓授权、交易所对账、总杠杆、同向持仓、OHLCV/时钟、订单状态机和原生止损等硬风控。

AI 增强模式中出现 API 鉴权失败、额度耗尽、超时或无效 JSON，仍必须 fail-closed/保守降级，不得静默切换成纯策略实盘开仓。真正停止所有新开仓必须使用暂停开仓或撤销标的授权。

由于这会改变真实交易行为，从旧语义发布到云端前，若云端当前 `ai.enabled=false`，必须先暂停新开仓；部署后由管理员明确决定是否重新授权。
...
python scripts\project_audit_board.py --mode full --include-cloud-runtime --cloud-host root@47.84.92.81 --fail-on-skipped --expect-cloud-live-ready --json-out output\audit\project_audit_board_full_cloud.json
```
...
- 云端趋势小资金灰度若继续使用弱密码，只能使用：

```powershell
python scripts\cloud_runtime_audit.py --host root@47.84.92.81 --key .ssh\aiquant_aliyun --expect-live-ready --runtime-env-mode trend-live --allow-weak-passwords
python scripts\project_audit_board.py --mode full --include-cloud-runtime --cloud-host root@47.84.92.81 --expect-cloud-live-ready --cloud-runtime-env-mode trend-live --cloud-allow-weak-passwords
```

  这只表示“账户1趋势实盘灰度可继续运行”。它不代表 `cloud-live` 多账户跟随、DeepSeek 主备、大资金无人值守或强密码公网验收通过。
