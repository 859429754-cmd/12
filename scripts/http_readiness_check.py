...
def main() -> int:
    ...
    try:
        ...
        if exc.code != 401:
            print(json.dumps({"ok": False, "mode": args.mode, "error_type": type(exc).__name__, "status": exc.code}, ensure_ascii=False))
            return 2
        try:
            cookie = login_cookie(args.base_url, timeout=args.timeout)
            if not cookie:
                raise exc
            payload = fetch_json(args.base_url.rstrip("/") + path, timeout=args.timeout, cookie=cookie)
        except (HTTPError, URLError, TimeoutError, json.JSONDecodeError) as login_exc:
            print(json.dumps({"ok": False, "mode": args.mode, "error_type": type(login_exc).__name__}, ensure_ascii=False))
            return 2
